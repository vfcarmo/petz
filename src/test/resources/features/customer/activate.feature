# language: en
@customer
Feature: Activate a customer by id
  User wants to activate a customer by id
  ...
  Scenario Outline: Activate a customer successfully
    ...
    When the user request activation of the customer '<customerId>'
  ...
  Scenario Outline: Try to activate a customer with nonexistent customer id
    Given the database is "online"
    When the user request activation of the customer '<customerId>'
  ...
  Scenario: Try to activate a customer when the database is offline
    Given the database is "offline"
    When the user request activation of the customer '684e5119-6c38-4b20-9122-00faa9c8b413'
